...
from teleop.utils.weighted_moving_filter import WeightedMovingFilter # 假设这个工具类存在
...
class X1_LeftArmIK:
    ...
    def __init__(self, urdf_path, visualization=False):
        ...
        cTf_target = casadi.SX.sym("tf_target", 4, 4) # Target pose (SE3)
        ...
        error_pos = self.cdata.oMf[self.ee_frame_id].translation - cTf_target[:3, 3]
        
        # Rotational error: difference between current EE orientation and target orientation
        error_rot = cpin.log3(self.cdata.oMf[self.ee_frame_id].rotation @ cTf_target[:3, :3].T)
        ...
        translational_cost = casadi.sumsqr(error_pos)
        rotation_cost = casadi.sumsqr(error_rot)
...
def zenoh_callback(sample):
    global current_target_position, current_angles_rad
    try:
        ...
        if np.all(np.isfinite(target_angles_rad)):
            ...
            # Update the current position based on the new angles for the next iteration
            new_fk_pose = ik_solver.forward_kinematics(current_angles_rad)
            current_target_position = new_fk_pose[:3, 3]
            
            # For logging, we can map the reduced model angles back to a full array if needed
            # This is an advanced step, for now we just log the active joint angles.
            active_joint_angles_deg = np.rad2deg(current_angles_rad)
            log.info(f"IK Solution (Active Joint Angles): {dict(zip(ik_solver.active_joints_names, active_joint_angles_deg))}")
            log.info(f"New FK Position: {current_target_position}")
        else:
...
if __name__ == "__main__":
    log.info("Starting Zenoh IK Controller with Pinocchio/CasADi...")
    
    conf = zenoh.Config()
    if ZENOH_ROUTER_ADDRESS:
        conf.insert_json5("connect", json.dumps({"endpoints": [ZENOH_ROUTER_ADDRESS]}))
